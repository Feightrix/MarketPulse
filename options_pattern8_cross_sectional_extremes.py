...
def generate_variant(v, sessions):
    ...
    for day in all_days:
        ...
        if len(prepared) < MIN_SYMBOLS_PER_EVAL_DAY:
            continue
        common_ts = sorted(set.intersection(*(set(b["ts"] for b in bars) for bars in prepared.values())))
        ...
        for ts in common_ts:
            ...
            if len(ranked) < MIN_SYMBOLS_PER_EVAL_DAY:
                continue
            high = max(ranked, key=lambda x: x[0])
            low = min(ranked, key=lambda x: x[0])
            # Choose whichever cross-sectional extreme is farther from the universe center.
            median_score = sorted(x[0] for x in ranked)[len(ranked)//2]
            ...
            entry_i = i + 1  # completed ranking bar, next-bar entry
...
def main():
    ...
    result = {
        ...
        "selection_note": "Relative-rank extremes; next-bar entry only. First 3 chronological folds select; fourth is untouched holdout.",
        "generated_utc": datetime.now(timezone.utc).isoformat(),
    }
    Path(p3.RESULT_JSON).write_text(json.dumps(result, indent=2, sort_keys=True) + "\n")
    lines = ["# MarketPulse — Pattern #8 Cross-Sectional Extremes", "", "**Research only. No orders. Next-bar entries only.**", "", f"Evaluation sessions: **{len(eval_days)}**", f"Selected development candidate: **{selected or 'NONE'}**", ""]
