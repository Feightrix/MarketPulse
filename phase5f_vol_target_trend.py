...
SMAS = [150, 200]
VOL_WINDOWS = [42, 63]
TARGET_VOLS = [0.04, 0.06, 0.08]
RISK_CAPS = [0.50, 0.75, 1.00]
...
def download_panel():
    ...
    for s in SYMS:
        v = c[s].dropna()
        if v.empty:
            raise RuntimeError(f"No history for {s}")
...
def target_weights(c, i, cfg):
    lb, sma_n, vw = cfg["lookback"], cfg["sma"], cfg["vol_window"]
    need = max(lb, sma_n, vw + 1)
    if i < need:
    ...
    rets = c[eligible].pct_change().iloc[i-vw+1:i+1].dropna()
    if len(rets) < max(20, vw // 2):
        return {DEFENSIVE: 1.0}

    vols = rets.std(ddof=1) * np.sqrt(252.0)
    ...
    base = inv / inv.sum()
    cov = rets[eligible].cov().to_numpy() * 252.0
    w = base.to_numpy(dtype=float)
    port_var = float(w @ cov @ w)
    port_vol = np.sqrt(max(port_var, 0.0))
    if not np.isfinite(port_vol) or port_vol <= 0:
        return {DEFENSIVE: 1.0}

    scale = min(cfg["risk_cap"], cfg["target_vol"] / port_vol)
    scale = max(scale, 0.0)
    out = {s: float(base[s] * scale) for s in eligible}
    out[DEFENSIVE] = max(0.0, 1.0 - scale)
    return out
...
def simulate(o,c,start,end,cfg,bps):
    ...
    for i in range(1,len(dates)):
        ...
        eq*=1.0+sum(w[s]*overnight[s] for s in w); w=drift(w,overnight)
        ...
        eq*=1.0+sum(w[s]*intra[s] for s in w); w=drift(w,intra)
        curve.append((d,eq))
...
def summarize(curve,events,turns):
    ...
    for y in sorted(s.index.year.unique()):
        ys=s[s.index.year==y]; prior=s[s.index<ys.index[0]]
    ...
    m=s.resample("ME").last(); mr=(m/m.shift(1)-1.0).dropna()
...
def dev_valid(m):
    return years_positive(m,range(2008,2015)) and m["cagr_pct"]>=3.0 and m["max_drawdown_pct"]<=8.0


def score(m):
    ys=list(m["annual_returns_pct"].values()); weakest=min(ys) if ys else -999.0
    median=float(np.median(ys)) if ys else -999.0
    return 4.0*weakest + 0.5*median + m["cagr_pct"] - 0.75*m["max_drawdown_pct"]
...
def main():
    print("Downloading Phase 5F history..."); o,c,coverage=download_panel()
    configs=[{"lookback":lb,"sma":sma,"vol_window":vw,"target_vol":tv,"risk_cap":cap}
             for lb,sma,vw,tv,cap in product(LOOKBACKS,SMAS,VOL_WINDOWS,TARGET_VOLS,RISK_CAPS)]
    candidates=[]
    for cfg in configs:
    ...
    valid=[x for x in candidates if x["dev_valid"]]
    selected=max(valid if valid else candidates,key=lambda x:x["score"]); cfg=selected["config"]
    ...
    result={"phase":"5F","strategy":"independent absolute-trend sleeves with inverse-vol weighting and portfolio vol targeting",
            "starting_equity":START_EQ,"universe":RISKY,"defensive_asset":DEFENSIVE,"coverage":coverage,
            "candidate_count":len(configs),"valid_development_candidates":len(valid),"selected_config":cfg,
            "development_2bps":d2,"development_10bps":selected["development_10bps"],"validation_2bps":v2,"validation_10bps":v10,
            "holdout1_2bps":h12,"holdout1_10bps":h110,"holdout2_2bps":h22,"holdout2_10bps":h210,
            "gate_checks":checks,"gate":"PASS" if passed else "FAIL","research_only":True}
    with open("phase5f_results.json","w") as f: json.dump(result,f,indent=2)
    failures=[k for k,v in checks.items() if not v]
    summary=f"""# MarketPulse Phase 5F — Volatility-Targeted Diversified Trend\n\n**Gate: {result['gate']}**\n\n## Architecture\nEach asset qualifies independently using its own trend. Qualified assets are inverse-volatility weighted, then the entire risky sleeve is scaled to a volatility target. Unused capital remains in BIL.\n\n## Selected configuration\n- Momentum lookback: **{cfg['lookback']} days**\n- Trend SMA: **{cfg['sma']} days**\n- Volatility window: **{cfg['vol_window']} days**\n- Portfolio target volatility: **{cfg['target_vol']*100:.1f}% annualized**\n- Maximum risky allocation: **{cfg['risk_cap']*100:.0f}%**\n- Rebalance: **monthly**\n\n## Development 2008–2014\n- Valid candidates: **{len(valid)} / {len(configs)}**\n- 2 bps: {fmt(d2)}\n- 10 bps: {fmt(selected['development_10bps'])}\n\n## Validation 2015–2019\n- 2 bps: {fmt(v2)}\n- 10 bps: {fmt(v10)}\n\n## Holdout 2020–2023\n- 2 bps: {fmt(h12)}\n- 10 bps: {fmt(h110)}\n\n## Final holdout 2024–2026 YTD\n- 2 bps: {fmt(h22)}\n- 10 bps: {fmt(h210)}\n\n## Gate checks\n"""
    ...
    with open("phase5f_summary.md","w") as f: f.write(summary)
    print(summary)
