...
STATE_FILE = "strategy2_state.json"
STATUS_FILE = "strategy2_status.json"
STATUS_MD = "strategy2_status.md"
LOG_FILE = "strategy2_log.jsonl"

# Initial side-by-side synchronization: Strategy 2 starts with the exact
# quantities produced by Strategy 1's frozen Phase 5H $2,500 rebalance on
# 2026-08-17. After bootstrap, Strategy 2 uses the same frozen engine.
CONTROL_BASELINE_SIGNAL_DATE = "2026-08-17"
CONTROL_BASELINE_QUANTITIES = {
    "BIL": 11.601578,
    "IWM": 0.859844,
    "QQQ": 0.206149,
    "SPY": 0.372274,
...
CONTROL_BASELINE_WEIGHTS = {
    "BIL": 0.425,
    "IWM": 0.10461685713681579,
    "QQQ": 0.06021541089438376,
    "SPY": 0.11509737409687856,
    "XLE": 0.08659929638829342,
    "XLK": 0.025,
    "XLP": 0.05847106148362846,
    "XLU": -0.025,
    "XLV": 0.025,
    "XLY": -0.025,
}
...
def represented_from_quantities(desired, equity, prices):
    return {
        sym: float(float(qty) * float(prices[sym]) / equity) if equity > 0 else 0.0
...
def write_status(status):
    ...
    lines = [
        ...
        f"- Experiment: **{status.get('experiment', 'BASELINE_CLONE')}**",
...
def main():
    ...
    try:
        ...
        status = {**base, "status": "WAITING_FOR_STRATEGY2_CREDENTIALS", "problems": [str(e)]}
        write_status(status)
        append_log(status)
        return
    ...
    if account.get("status") != "ACTIVE" or account.get("trading_blocked"):
        status = {**common, "status": "PAPER_ACCOUNT_BLOCKED", "problems": ["Strategy 2 paper account is not active/tradable"]}
        write_status(status)
        append_log(status)
        return

    state = load_state()
    experiment = state.get("experiment", "BASELINE_CLONE")
    common["experiment"] = experiment

    try:
        positions = core.current_positions()
        pending = [o for o in core.open_orders() if str(o.get("client_order_id", "")).startswith("mps2-")]
    except Exception as e:
    ...
    if not state.get("initialized"):
        if not (BOOTSTRAP_MIN <= equity <= BOOTSTRAP_MAX):
            status = {**common, "status": "WRONG_PAPER_ACCOUNT", "problems": [f"First-run equity must be about $2,500; received ${equity:,.2f}."]}
            write_status(status)
            append_log(status)
            return
        if positions or pending:
            status = {**common, "status": "BOOTSTRAP_ACCOUNT_NOT_EMPTY", "problems": ["Strategy 2 must begin with no positions and no MarketPulse Strategy 2 orders."]}
            write_status(status)
            append_log(status)
            return

    if equity < OPERATING_FLOOR:
        status = {**common, "status": "OPERATING_FLOOR_BLOCK", "problems": [f"Equity ${equity:,.2f} is below the ${OPERATING_FLOOR:,.2f} MarketPulse operating floor."]}
        write_status(status)
        append_log(status)
        return
    ...
    try:
        ...
        metadata = {s: core.asset(s) for s in CONTROL_BASELINE_WEIGHTS}
    except Exception as e:
        status = {**common, "status": "MARKET_DATA_OR_ASSET_READ_FAILED", "problems": [str(e)]}
        write_status(status)
        append_log(status)
        return

    if not state.get("initialized"):
        weights = dict(CONTROL_BASELINE_WEIGHTS)
        desired = dict(CONTROL_BASELINE_QUANTITIES)
        represented = represented_from_quantities(desired, equity, prices)
        fit = capital_fit(weights, represented, equity)
        broker_problems = core.preflight(weights, equity, metadata, desired)
        signal_date = CONTROL_BASELINE_SIGNAL_DATE
        trend = None
        neutral = None
        common["baseline_source"] = "Strategy 1 control quantities from 2026-08-17"
    else:
        try:
    ...
    if pending:
        status = {**common, "status": "PENDING_STRATEGY2_ORDERS", "pending_orders": pending}
        write_status(status)
        append_log(status)
        return
    ...
    try:
        def submit_strategy2(sym, qty, side, tag):
            return original_submit(sym, qty, side, tag.replace("mp6b-", "mps2-", 1))

        core.submit_market = submit_strategy2
        ...
        state.update({
            ...
            "baseline_control_signal_date": CONTROL_BASELINE_SIGNAL_DATE,
        ...
        save_state(state)
        status = {**common, "status": "REBALANCE_COMPLETE", "actions": actions, "final_positions": final_positions, "rebalance_count": state["rebalance_count"]}
    except Exception as e:
